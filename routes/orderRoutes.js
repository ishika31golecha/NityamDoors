...
router.post('/', protect, authorize('SalesExecutive', 'FactoryAdmin', 'SuperAdmin'), upload.single('doorFile'), async (req, res) => {
  try {
    ...
    customerType = customerTypeMap[customerType] || customerType;
...
router.get('/', protect, async (req, res) => {
  try {
    ...
    console.log('All orders count:', Array.isArray(orders) ? orders.length : 0);
...
/**
 * @route   GET /api/orders/completed-orders
 * @desc    Get only completed orders
 * @access  Private
 */
router.get('/completed-orders', protect, async (req, res) => {
  try {
    let orders = await Order.find({
      status: { $regex: '^completed$', $options: 'i' }
    })
      .select('_id orderId status customer.name customer.mobileNumber customer.address createdAt')
      .sort({ createdAt: -1 })
      .lean();

    console.log('Completed orders count:', orders.length);

    // Fallback: use production stage data when no completed orders are found.
    if (!orders.length) {
      const productionOrderIds = await DoorUnit.distinct('orderId', {
        currentStage: 'DELIVERY_PENDING'
      });

      console.log('Fallback production orderIds count:', productionOrderIds.length);

      orders = await Order.find({ orderId: { $in: productionOrderIds } })
        .select('_id orderId customer.name customer.mobileNumber customer.address createdAt')
        .sort({ createdAt: -1 })
        .lean();
    }
